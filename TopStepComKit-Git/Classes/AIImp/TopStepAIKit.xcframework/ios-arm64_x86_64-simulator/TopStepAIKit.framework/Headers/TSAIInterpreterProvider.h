...
/**
 * @brief AI Interpreter provider protocol
 * @chinese AI 同声传译接口协议
 *
 * @discussion
 ...
 *
 *       Shape parallels AI assistant chat:
 ...
 */
@protocol TSAIInterpreterProvider <NSObject>

/**
 * @brief Whether simultaneous interpretation is supported
 * @chinese 是否支持同声传译
 *
 * @return
 * EN: YES when simultaneous interpretation is supported
 * CN: 支持同声传译时返回 YES
 */
...
/**
 * @brief Register listener for interpreter session state changes
 * @chinese 注册同声传译会话状态变化监听
 *
 * @param stateBlock
 * EN: Callback invoked when session state changes; pass nil to unregister
