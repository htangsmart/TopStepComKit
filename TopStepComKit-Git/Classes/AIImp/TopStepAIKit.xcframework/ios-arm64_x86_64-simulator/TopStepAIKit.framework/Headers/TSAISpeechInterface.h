 ...
 */
@protocol TSAISpeechInterface <NSObject>
...
/**
 * @brief Recognize a complete PCM audio buffer
 * @chinese 识别一段完整 PCM 音频
 *
 * @param pcmData
 * EN: Non-empty 16 kHz, mono, signed Int16 little-endian PCM data
 * CN: 非空的 16 kHz、单声道、有符号 Int16 小端 PCM 数据
 * @param config EN: Recognition language configuration. CN: 识别语言配置。
 * @param onPartialResult EN: Optional cumulative partial callback. CN: 可选的累积中间结果回调。
 * @param completion EN: Main-thread asynchronous completion invoked exactly once. CN: 主线程异步且仅调用一次的完成回调。
