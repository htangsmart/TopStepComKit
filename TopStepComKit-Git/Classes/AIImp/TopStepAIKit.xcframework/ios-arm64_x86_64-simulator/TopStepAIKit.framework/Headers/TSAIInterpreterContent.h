 ...
 *       `audioChunk` is a **delta**. Append audio to the App playback buffer
 *       only when `autoPlayVoice` is `NO`; when it is `YES`, use AudioChunk
 *       only for observation or storage and do not play it again.
 *
 ...
 *       | AudioChunk      |       ●        | ● (目标) |  —   |      —      |     ●      |      ●      |      ●       |
 *
 *       注意：`text` 为**累积**值（可直接赋值给 UI），`audioChunk` 为**增量**片段。
 *       仅当 `autoPlayVoice` 为 `NO` 时才追加到 App 播放缓冲；为 `YES` 时
 *       AudioChunk 仅用于观察或存储，不得再次播放。
 *
...
/**
 * @brief Translation TTS audio delta segment
 * @chinese 译文 TTS 音频增量片段
 *
 * @discussion
 ...
 *       actual format. Play it in the App only when `autoPlayVoice` is `NO`.
 *       When `autoPlayVoice` is `YES`, SDK playback already owns the route.
 * [CN]: 自本段 utterance 上一次 AudioChunk 回调以来新合成的音频字节。
 *       默认格式为 16 kHz / 16-bit / 单声道 PCM 小端；
 *       具体格式以 `audioFormat` 为准。仅当 `autoPlayVoice` 为 `NO` 时由 App 播放；
 *       为 `YES` 时 SDK 已持有播放链路，App 不得再次播放。
 */
