...
typedef NS_ENUM(NSInteger, TSAILanguage) {
    ...
    /// 俄语 (Russian, source ru / target ru-RU)
    TSAILanguageRussian            = 10,
    /// 葡萄牙语 (Portuguese, pt-BR)
    TSAILanguagePortuguese         = 11,
    /// 印度尼西亚语 (Indonesian, id-ID)
    TSAILanguageIndonesian         = 12,
    /// 马来语 (Malay, ms-MY)
    TSAILanguageMalay              = 13,
    /// 菲律宾语 (Filipino, fil-PH)
    TSAILanguageFilipino           = 14,
    /// 泰语 (Thai, th-TH)
    TSAILanguageThai               = 15,
    /// 阿拉伯语 (Arabic, ar-SA)
    TSAILanguageArabic             = 16,
    /// 越南语 (Vietnamese, vi)
    TSAILanguageVietnamese         = 17,
    /// 丹麦语 (Danish, da)
    TSAILanguageDanish             = 18,
    /// 希腊语 (Greek, el)
    TSAILanguageGreek              = 19,
    /// 芬兰语 (Finnish, fi)
    TSAILanguageFinnish            = 20,
    /// 印地语 (Hindi, hi)
    TSAILanguageHindi              = 21,
    /// 意大利语 (Italian, it)
    TSAILanguageItalian            = 22,
    /// 荷兰语 (Dutch, nl)
    TSAILanguageDutch              = 23,
    /// 挪威语 (Norwegian, no)
    TSAILanguageNorwegian          = 24,
    /// 波兰语 (Polish, pl)
    TSAILanguagePolish             = 25,
    /// 瑞典语 (Swedish, sv)
    TSAILanguageSwedish            = 26,
    /// 斯瓦希里语 (Swahili, sw)
    TSAILanguageSwahili            = 27,
    /// 土耳其语 (Turkish, tr)
    TSAILanguageTurkish            = 28,
    /// 孟加拉语 (Bengali, bn)
    TSAILanguageBengali            = 29,
    /// 罗马尼亚语 (Romanian, ro)
    TSAILanguageRomanian           = 30,
    /// 尼泊尔语 (Nepali, ne)
    TSAILanguageNepali             = 31,
    /// 乌克兰语 (Ukrainian, uk)
    TSAILanguageUkrainian          = 32,
    /// 捷克语 (Czech, cs)
    TSAILanguageCzech              = 33,
    /// 威尔士语 (Welsh, cy)
    TSAILanguageWelsh              = 34,
    /// 匈牙利语 (Hungarian, hu)
    TSAILanguageHungarian          = 35,
    /// 冰岛语 (Icelandic, is)
    TSAILanguageIcelandic          = 36,
    /// 格鲁吉亚语 (Georgian, ka)
    TSAILanguageGeorgian           = 37,
    /// 哈萨克语 (Kazakh, kk)
    TSAILanguageKazakh             = 38,
    /// 卢森堡语 (Luxembourgish, lb)
    TSAILanguageLuxembourgish      = 39,
    /// 拉脱维亚语 (Latvian, lv)
    TSAILanguageLatvian            = 40,
    /// 马拉雅拉姆语 (Malayalam, ml)
    TSAILanguageMalayalam          = 41,
    /// 斯洛伐克语 (Slovak, sk)
    TSAILanguageSlovak             = 42,
    /// 斯洛文尼亚语 (Slovenian, sl)
    TSAILanguageSlovenian          = 43,
    /// 塞尔维亚语 (Serbian, sr)
    TSAILanguageSerbian            = 44,
    /// 泰卢固语 (Telugu, te)
    TSAILanguageTelugu             = 45,
    /// 加泰罗尼亚语 (Catalan, ca)
    TSAILanguageCatalan            = 46,
    /// 泰米尔语 (Tamil, ta)
    TSAILanguageTamil              = 47,
    /// 乌尔都语 (Urdu, ur)
    TSAILanguageUrdu               = 48,
    /// 克罗地亚语 (Croatian, hr)
    TSAILanguageCroatian           = 49,
    /// 保加利亚语 (Bulgarian, bg)
    TSAILanguageBulgarian          = 50,
    /// 立陶宛语 (Lithuanian, lt)
    TSAILanguageLithuanian         = 51,
    /// 拉丁语 (Latin, la)
    TSAILanguageLatin              = 52,
    /// 毛利语 (Maori, mi)
    TSAILanguageMaori              = 53,
    /// 波斯语 (Persian, fa)
    TSAILanguagePersian            = 54,
    /// 阿塞拜疆语 (Azerbaijani, az)
    TSAILanguageAzerbaijani        = 55,
    /// 卡纳达语 (Kannada, kn)
    TSAILanguageKannada            = 56,
    /// 爱沙尼亚语 (Estonian, et)
    TSAILanguageEstonian           = 57,
    /// 马其顿语 (Macedonian, mk)
    TSAILanguageMacedonian         = 58,
    /// 布列塔尼语 (Breton, br)
    TSAILanguageBreton             = 59,
    /// 巴斯克语 (Basque, eu)
    TSAILanguageBasque             = 60,
    /// 亚美尼亚语 (Armenian, hy)
    TSAILanguageArmenian           = 61,
    /// 蒙古语 (Mongolian, mn)
    TSAILanguageMongolian          = 62,
    /// 波斯尼亚语 (Bosnian, bs)
    TSAILanguageBosnian            = 63,
    /// 阿尔巴尼亚语 (Albanian, sq)
    TSAILanguageAlbanian           = 64,
    /// 加利西亚语 (Galician, gl)
    TSAILanguageGalician           = 65,
    /// 马拉地语 (Marathi, mr)
    TSAILanguageMarathi            = 66,
    /// 旁遮普语 (Punjabi, pa)
    TSAILanguagePunjabi            = 67,
    /// 僧伽罗语 (Sinhala, si)
    TSAILanguageSinhala            = 68,
    /// 高棉语 (Khmer, km)
    TSAILanguageKhmer              = 69,
    /// 绍纳语 (Shona, sn)
    TSAILanguageShona              = 70,
    /// 约鲁巴语 (Yoruba, yo)
    TSAILanguageYoruba             = 71,
    /// 索马里语 (Somali, so)
    TSAILanguageSomali             = 72,
    /// 南非荷兰语 (Afrikaans, af)
    TSAILanguageAfrikaans          = 73,
    /// 奥克语 (Occitan, oc)
    TSAILanguageOccitan            = 74,
    /// 白俄罗斯语 (Belarusian, be)
    TSAILanguageBelarusian         = 75,
    /// 塔吉克语 (Tajik, tg)
    TSAILanguageTajik              = 76,
    /// 信德语 (Sindhi, sd)
    TSAILanguageSindhi             = 77,
    /// 古吉拉特语 (Gujarati, gu)
    TSAILanguageGujarati           = 78,
    /// 阿姆哈拉语 (Amharic, am)
    TSAILanguageAmharic            = 79,
    /// 意第绪语 (Yiddish, yi)
    TSAILanguageYiddish            = 80,
    /// 老挝语 (Lao, lo)
    TSAILanguageLao                = 81,
    /// 乌兹别克语 (Uzbek, uz)
    TSAILanguageUzbek              = 82,
    /// 法罗语 (Faroese, fo)
    TSAILanguageFaroese            = 83,
    /// 海地克里奥尔语 (Haitian Creole, ht)
    TSAILanguageHaitianCreole      = 84,
    /// 普什图语 (Pashto, ps)
    TSAILanguagePashto             = 85,
    /// 土库曼语 (Turkmen, tk)
    TSAILanguageTurkmen            = 86,
    /// 挪威尼诺斯克语 (Norwegian Nynorsk, nn)
    TSAILanguageNorwegianNynorsk   = 87,
    /// 马耳他语 (Maltese, mt)
    TSAILanguageMaltese            = 88,
    /// 梵语 (Sanskrit, sa)
    TSAILanguageSanskrit           = 89,
    /// 缅甸语 (Burmese, my)
    TSAILanguageBurmese            = 90,
    /// 藏语 (Tibetan, bo)
    TSAILanguageTibetan            = 91,
    /// 马达加斯加语 (Malagasy, mg)
    TSAILanguageMalagasy           = 92,
    /// 阿萨姆语 (Assamese, as)
    TSAILanguageAssamese           = 93,
    /// 鞑靼语 (Tatar, tt)
    TSAILanguageTatar              = 94,
    /// 夏威夷语 (Hawaiian, haw)
    TSAILanguageHawaiian           = 95,
    /// 林加拉语 (Lingala, ln)
    TSAILanguageLingala            = 96,
    /// 豪萨语 (Hausa, ha)
    TSAILanguageHausa              = 97,
    /// 巴什基尔语 (Bashkir, ba)
    TSAILanguageBashkir            = 98,
    /// 爪哇语 (Javanese, jw)
    TSAILanguageJavanese           = 99,
    /// 巽他语 (Sundanese, su)
    TSAILanguageSundanese          = 100,
};
