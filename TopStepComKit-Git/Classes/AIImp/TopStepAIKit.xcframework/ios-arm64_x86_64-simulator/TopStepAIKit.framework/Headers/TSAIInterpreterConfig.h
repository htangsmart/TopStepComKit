 ...
 *       audio is produced and surfaced to the App. SDK playback ownership
 *       is controlled by `autoPlayVoice`, and its phone-loudspeaker preference
 *       is controlled by `preferSpeakerOutput`.
 *
 ...
 *       是否由 SDK 托管播放由 `autoPlayVoice` 控制，是否优先使用手机扬声器
 *       由 `preferSpeakerOutput` 控制。
 */
@property (nonatomic, assign) BOOL enableVoiceOutput;

/**
 * @brief Whether the SDK automatically plays the translated audio
 * @chinese 是否由 SDK 自动播放译文音频
 *
 * @discussion
 * [EN]: When `YES`, the SDK owns translated-audio playback and the App must not
 *       play the AudioChunk stream again. On the AIBuds internal-audio path,
 *       SDK-managed playback is also used as the duplex AEC reference.
 ...
 *       it through whatever audio route it prefers. AEC is not guaranteed
 *       for App-managed playback.
 *
 ...
 *
 * [CN]: 为 `YES` 时，由 SDK 负责播放译文音频，App 不得再次播放 AudioChunk 流。
 *       AIBuds 内部音频链路还会将 SDK 播放作为双工 AEC 的参考信号。
 *       为 `NO` 时 SDK 不播放任何音频，App 须自行从 `onContent` 的
 *       AudioChunk 流取数据并按需播放；App 自行播放的链路不保证具备 AEC。
 *
...
/**
 * @brief Whether SDK-managed playback prefers the phone loudspeaker
 * @chinese SDK 托管播放是否优先使用手机扬声器
 *
 * @discussion
 * [EN]: Applies when `enableVoiceOutput` and `autoPlayVoice` are both enabled.
 *       Default is `YES`.
 * [CN]: 当 `enableVoiceOutput` 与 `autoPlayVoice` 均开启时生效。
 *       默认为 `YES`。
 */
@property (nonatomic, assign) BOOL preferSpeakerOutput;
 ...
 *     `autoPlayVoice = YES`, `preferSpeakerOutput = YES`. Both language fields are
