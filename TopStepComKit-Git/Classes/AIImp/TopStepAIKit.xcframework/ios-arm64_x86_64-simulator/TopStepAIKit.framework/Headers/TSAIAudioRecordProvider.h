...
/**
 * @brief Audio recording provider
 * @chinese 录音功能接口
 *
 * @discussion
 ...
 */
@protocol TSAIAudioRecordProvider <NSObject>

/**
 * @brief Check whether AI audio recording is supported
 * @chinese 检查是否支持 AI 录音
 *
 * @return
 * EN: YES if the current platform and device support AI audio recording
 * CN: 当前平台与设备支持 AI 录音时返回 YES
 */
- (BOOL)isSupportAIAudioRecording;

/**
 * @brief Check whether call audio recording is supported
 * @chinese 检查是否支持通话录音
 *
 * @return
 * EN: YES if the current platform and device support call-scene audio recording
 * CN: 当前平台与设备支持通话场景录音时返回 YES
 */
- (BOOL)isSupportCallAudioRecording;

/**
 * @brief Check whether normal audio recording is supported
 * @chinese 检查是否支持普通录音
 *
 * @return
 * EN: YES if the current platform and device support normal audio recording
 * CN: 当前平台与设备支持普通录音时返回 YES
 */
- (BOOL)isSupportNormalAudioRecording;
...
/**
 * @brief Handle a device request to start AI recording
 * @chinese 处理设备请求开始 AI 录音事件
 *
 * @param scene
 * EN: Requested recording scene
 * CN: 请求的录音场景
 */
- (void)handleDeviceRequestToStartAudioRecordingWithScene:(TSAIAudioRecordScene)scene;

/**
 * @brief Handle a device request to stop AI recording
 * @chinese 处理设备请求停止 AI 录音事件
 */
- (void)handleDeviceRequestToStopAudioRecording;

/**
 * @brief Handle a device-side AI recording interruption
 * @chinese 处理设备侧 AI 录音中断事件
 *
 * @param reason
 * EN: Normalized interruption reason
 * CN: 标准化中断原因
 */
- (void)handleDeviceAudioRecordingInterruptionWithReason:(TSAIAudioRecordInterruptReason)reason;

/**
 * @brief Handle incremental device recording audio
 * @chinese 处理设备侧增量录音音频
 *
 * @param opusData
 ...
 * CN: 可用时的 PCM 音频数据
 */
- (void)handleDeviceAudioRecordingDataWithOpusData:(nullable NSData *)opusData
