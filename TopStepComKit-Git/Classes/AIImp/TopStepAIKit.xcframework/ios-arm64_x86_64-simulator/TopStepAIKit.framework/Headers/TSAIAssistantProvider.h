...
/**
 * @brief AI Assistant provider protocol
 * @chinese AI 助手接口协议
 *
 * @discussion
 * [EN]: Defines AI assistant capabilities. Two shapes coexist:
 *
 *       1. Text summarization — a single-shot streaming task returning a
 *          final result and cancellable through its task identifier.
 ...
 *
 *       1. 文本总结 —— 单次流式返回最终结果，可通过任务标识取消。
 *       2. 语音对话会话 —— 由设备麦克风音频驱动的端到端长会话，
 *          会话内由 VAD 自动断句产生多轮问答。例：AI 对话。
 *          会话结束方式为 `stopChatWithTaskId:`（优雅冲刷）或自动超时，
 *          不提供单独的强制取消入口。
 */
@protocol TSAIAssistantProvider <NSObject>

#pragma mark - AI Summary

/**
 * @brief Indicates whether AI summary is supported
 * @chinese 指示是否支持 AI 总结功能
 *
 * @return
 * EN: YES if AI summary is supported; otherwise NO
 * CN: 支持 AI 总结时返回 YES，否则返回 NO
 *
 * @discussion
 * [EN]: Callers should use this method before starting summary tasks.
 * [CN]: 调用方在启动 Summary 任务前应使用本方法判断 AISummary 能力。
 */
- (BOOL)isSupportAISummary;
...
/**
 * @brief Indicates whether AI chat is supported by the connected device
 * @chinese 指示当前连接设备是否支持 AI 对话功能
 *
 * @return
 * EN: YES if AI chat is supported; otherwise NO
 * CN: 支持 AI 对话时返回 YES，否则返回 NO
 *
 * @discussion
 * [EN]: Callers should use this method before starting chat sessions or
 *       registering chat events.
 * [CN]: 调用方在启动 AI 对话或注册对话事件前应使用本方法判断 AIChat 能力。
 */
- (BOOL)isSupportAIChat;
...
/**
 * @brief Register listener for AI chat session state changes
 * @chinese 注册 AI 对话会话状态变化监听
 *
 * @param stateBlock
 * EN: Callback invoked when chat session state changes; pass nil to unregister
 * CN: 对话会话状态变化时触发的回调；传 nil 可取消监听
 *
 * @discussion
 * [EN]: This is an app-facing state stream compatible with Flutter's historical
 *       0 / 1 / 2 / 3 / 255 state vocabulary. Device request events remain
 *       available through `registerOnAIChatDeviceEvent:`.
 * [CN]: 这是面向 App 的状态流，兼容 Flutter 历史使用的
 *       0 / 1 / 2 / 3 / 255 状态语义。设备请求事件仍通过
 *       `registerOnAIChatDeviceEvent:` 下发。
 */
- (void)registerAIChatStateDidChanged:(TSAIChatStateBlock _Nullable)stateBlock;

/**
 * @brief Handle a device-side AI chat session event
 * @chinese 处理设备侧 AI 对话会话事件
 *
 * @param event
 * EN: Normalized chat event
 * CN: 标准化对话事件
 *
 * @param audioChannel
 * EN: Audio channel selected by the device
 * CN: 设备选择的音频通道
 */
- (void)handleDeviceChatSessionEvent:(TSAIChatDeviceEvent)event
                        audioChannel:(TSAIDeviceBridgeChatAudioChannel)audioChannel;

/**
 * @brief Handle incremental device chat audio
 * @chinese 处理设备侧增量对话音频
 *
 * @param opusData
 * EN: Opus audio data, when available
 * CN: 可用时的 Opus 音频数据
 *
 * @param pcmData
 * EN: PCM audio data, when available
 * CN: 可用时的 PCM 音频数据
 */
- (void)handleDeviceChatAudioWithOpusData:(nullable NSData *)opusData
