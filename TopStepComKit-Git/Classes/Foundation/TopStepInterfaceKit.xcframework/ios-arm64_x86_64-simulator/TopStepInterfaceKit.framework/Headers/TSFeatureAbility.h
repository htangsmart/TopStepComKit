...
typedef NS_OPTIONS(uint64_t, TSPeripheralSupportAbility) {
    ...
    /// 音乐 (Music)
    TSPeripheralSupportMusic             = 1ULL << 19,
    ...
    TSPeripheralSupportWorkoutPush       = 1ULL << 34,
 ...
 */
@property (nonatomic, readonly) BOOL isSupportMusic;
...
/**
 * @brief Indicates if sport push is supported
 * @chinese 指示是否支持运动上传功能
 */
@property (nonatomic, readonly) BOOL isSupportWorkoutPush;
...
/**
 * @brief Check whether all specified capabilities are supported
 * @chinese 检查是否支持指定的全部能力
 *
 * @param ability
 * EN: A single capability flag or a bitwise combination of capability flags
 * CN: 单个能力标志或多个能力标志的按位组合
 *
 * @return
 * EN: YES only when every specified capability is supported, otherwise NO
 * CN: 仅当指定的全部能力均受支持时返回YES，否则返回NO
 *
 * @discussion
 * EN: Passing TSPeripheralSupportNone returns NO. This method queries only the normalized
 *     supportCapabilities flags and does not interpret bit positions in originAbility.
 *     Use isCapabilitySupportedAtBitIndex: when a protocol-defined raw bit must be checked.
 * CN: 传入TSPeripheralSupportNone时返回NO。本方法仅查询归一化后的supportCapabilities标志，
 *     不解释originAbility中的原始位；协议定义的原始能力位应使用isCapabilitySupportedAtBitIndex:查询。
 */
- (BOOL)isSupportAbility:(TSPeripheralSupportAbility)ability;
