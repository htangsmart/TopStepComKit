...
/**
 * @brief Convert a message type to a FitCloud notification app
 * @chinese 将消息类型转换为 FitCloud 通知应用
 *
 * @param messageType
 * EN: Message type to convert
 * CN: 待转换的消息类型
 *
 * @return
 * EN: The matching FitCloud notification app, or nil when the type cannot be converted
 * CN: 对应的 FitCloud 通知应用；类型无法转换时返回 nil
 */
+ (nullable FitCloudNotificationApp)notificationAppForMessageType:(TSMessageType)messageType;
