 ...
 *       - data: Real-time temperature measurement data, nil if error occurs
 *       - error: Error information if data reception fails, nil if successful
 * [CN]: 接收实时测量数据的回调块
 *       - data: 实时温度测量数据，发生错误时为nil
 ...
 * @chinese 检查设备是否支持配置体温监测时间段
 *
 * @return
 * [EN]: YES if the device supports configuring start/end time for temperature monitoring, NO otherwise
 * [CN]: 如果设备支持配置体温监测开始/结束时间返回YES，否则返回NO
 *
 * @discussion
...
/**
 * @brief Synchronize raw temperature data within a specified time range
 * @chinese 同步指定时间范围内的原始温度数据
 *
 * @param startTime
 ...
 *
 * @param completion
 * [EN]: Completion block with synchronized raw temperature measurement items or error
 * [CN]: 包含同步的原始温度测量条目或错误的完成回调块
 */
...
/**
 * @brief Synchronize raw temperature data from a specified start time until now
 * @chinese 从指定开始时间同步至今的原始温度数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970)
 * [CN]: 数据同步的开始时间（1970年以来的秒数时间戳）
...
/**
 * @brief Synchronize daily temperature data within a specified time range
 * @chinese 同步指定时间范围内的每日温度数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970).
 ...
 *
 * @param completion
 * [EN]: Completion block with synchronized daily temperature models or error.
 *       Each TSTempDailyModel represents one day's aggregated data.
 * [CN]: 完成回调，返回同步的每日温度模型数组或错误。
 *       每个 TSTempDailyModel 代表一天的数据集合。
 *
 * @discussion
 * [EN]: This method synchronizes daily aggregated temperature data within the given time range.
...
/**
 * @brief Synchronize daily temperature data from a specified start time until now
 * @chinese 从指定开始时间同步至今的每日温度数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970).
 *       Will be automatically normalized to 00:00:00 of the specified day.
 ...
 *
 * @discussion
 * [EN]: This method synchronizes daily aggregated temperature data from the start time to the current time.
