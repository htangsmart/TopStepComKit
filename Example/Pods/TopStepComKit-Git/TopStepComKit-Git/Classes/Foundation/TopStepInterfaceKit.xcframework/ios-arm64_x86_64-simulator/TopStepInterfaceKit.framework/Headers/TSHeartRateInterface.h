...
/**
 * @brief Check if the device supports resting heart rate alert
 * @chinese 检查设备是否支持安静心率预警
 *
 * @return
 * [EN]: YES if the device supports alerting when resting heart rate is abnormal, NO otherwise
 * [CN]: 如果设备支持安静心率异常时预警返回YES，否则返回NO
 *
 * @discussion
 * [EN]: When YES, the restHRAlert property of TSAutoMonitorHRConfigs is effective —
 *       you can configure upper and lower limits for resting heart rate alerts.
 * [CN]: 返回YES时，TSAutoMonitorHRConfigs 的 restHRAlert 属性有效，可配置安静心率预警上下限。
 */
- (BOOL)isSupportRestHRAlert;

/**
 * @brief Check if the device supports exercise heart rate alert
 * @chinese 检查设备是否支持运动心率预警
 *
 * @return
 * [EN]: YES if the device supports alerting when exercise heart rate is abnormal, NO otherwise
 * [CN]: 如果设备支持运动心率异常时预警返回YES，否则返回NO
 *
 * @discussion
 * [EN]: When YES, the exerciseHRAlert property of TSAutoMonitorHRConfigs is effective —
 *       you can configure upper and lower limits for exercise heart rate alerts.
 * [CN]: 返回YES时，TSAutoMonitorHRConfigs 的 exerciseHRAlert 属性有效，可配置运动心率预警上下限。
 */
- (BOOL)isSupportExerciseHRAlert;

/**
 * @brief Check if the device supports configuring monitor time range
 * @chinese 检查设备是否支持配置心率监测时间段
 *
 * @return
 * [EN]: YES if the device supports configuring start/end time for heart rate monitoring, NO otherwise
 * [CN]: 如果设备支持配置心率监测开始/结束时间返回YES，否则返回NO
 *
 * @discussion
