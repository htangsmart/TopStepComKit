 ...
 *       - data: Real-time ECG measurement data, nil if error occurs
 *       - error: Error information if data reception fails, nil if successful
 * [CN]: 接收实时测量数据的回调块
 *       - data: 实时心电图测量数据，发生错误时为nil
...
/**
 * @brief Synchronize raw ECG data within a specified time range
 * @chinese 同步指定时间范围内的原始心电图数据
 *
 * @param startTime
 ...
 *
 * @param completion
 * [EN]: Completion block with synchronized raw ECG measurement items or error
 * [CN]: 包含同步的原始心电图测量条目或错误的完成回调块
 */
...
/**
 * @brief Synchronize raw ECG data from a specified start time until now
 * @chinese 从指定开始时间同步至今的原始心电图数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970)
 * [CN]: 数据同步的开始时间（1970年以来的秒数时间戳）
...
/**
 * @brief Synchronize daily ECG data within a specified time range
 * @chinese 同步指定时间范围内的每日心电图数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970).
 ...
 *
 * @param completion
 * [EN]: Completion block with synchronized daily ECG models or error.
 *       Each TSECGDailyModel represents one day's aggregated data.
 * [CN]: 完成回调，返回同步的每日心电图模型数组或错误。
 *       每个 TSECGDailyModel 代表一天的数据集合。
 *
 * @discussion
 * [EN]: This method synchronizes daily aggregated ECG data within the given time range.
...
/**
 * @brief Synchronize daily ECG data from a specified start time until now
 * @chinese 从指定开始时间同步至今的每日心电图数据
 *
 * @param startTime
 * [EN]: Start time for data synchronization (timestamp in seconds since 1970).
 *       Will be automatically normalized to 00:00:00 of the specified day.
 ...
 *
 * @discussion
 * [EN]: This method synchronizes daily aggregated ECG data from the start time to the current time.
