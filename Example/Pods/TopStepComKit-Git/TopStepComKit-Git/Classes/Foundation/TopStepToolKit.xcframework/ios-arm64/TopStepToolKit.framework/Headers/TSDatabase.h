...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define TSDBAutorelease(__v) ([__v autorelease]);
    #define TSDBReturnAutoreleased FMDBAutorelease

    #define TSDBRetain(__v) ([__v retain]);
    #define TSDBReturnRetained FMDBRetain

    #define TSDBRelease(__v) ([__v release]);

    #define TSDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define TSDBAutorelease(__v)
    #define TSDBReturnAutoreleased(__v) (__v)

    #define TSDBRetain(__v)
    #define TSDBReturnRetained(__v) (__v)

    #define TSDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define TSDBDispatchQueueRelease(__v)
    #else
        #define TSDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef NS_ENUM(int, TSDBCheckpointMode) {
    TSDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    TSDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    TSDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    TSDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
- (TSResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (TSResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (TSResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (TSResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (TSResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (TSResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;

/// Prepare SQL statement.
///
/// @param sql SQL statement to prepare, generally with `?` placeholders.

- (TSResultSet *)prepare:(NSString *)sql;
 ...
 */
- (BOOL)checkpoint:(TSDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(TSDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(TSDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
- (TSSqliteValueType)valueType:(void *)argv;
